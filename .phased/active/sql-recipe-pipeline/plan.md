...
- [x] **Phase 8**: Grammar reference document generated by introspection
  > Done: contract tests `tests/test_wf_phase8_doc.py` 4/4 green, `pytest -q` -> 80
  > passed exit 0, `ruff check src tests` clean. `docs/grammar.md` regenerates
  > byte-identically from the live grammar.
  > Note: element signatures are NOT reachable from the `_class_schema` nodes
  > (they carry `call_args_validations`/`declared_names`, no `_func`), so the
  > generator resolves each tag's declarative marker along `SqlBuilder.__mro__`
  > and reads `marker._func` / `marker._decorator` (for `collection_key`).
  > Plane per parameter comes from the docstring `Args:` marker (`semantic;`
  > -> semantic, else physical), which the doc then strips from the prose.
  > Files: src/genro_sql/modern/grammar_doc.py, docs/grammar.md,
  > tests/test_wf_phase8_doc.py
...
- [x] **Phase 10**: Shared helpers dedup and docstring notes (from review.md findings 2, 3, 4, 6)
  ...
  - Details: create common.py with the two symbols and `# wf:phase-10:new` markers; replace the three local definitions with the alias imports; add the two docstring notes; no behavior change except reader's list branch gaining `.strip()`.
  - Done: `pytest -q` exits 0 (80 passed) and `ruff check .` clean; `grep -rn "def _names" src/` returns nothing (only the shared `split_names` exists); `grep -rn "_INDEX_OPTIONS = (" src/` returns nothing outside common.py.
  - Run: opus / low
